...
-- Claude Code spend, aggregated per DAY (and per user).
...
--
-- Identity: prefers the human-readable `user_email` (set via OTEL_RESOURCE_ATTRIBUTES
-- + collector transform); falls back to the hashed user.id for older telemetry.

WITH events AS (
  SELECT
    timestamp,
    COALESCE(
      JSON_VALUE(labels, '$.user_email'),
      CONCAT('id:', JSON_VALUE(labels, '$."user.id"'))
    )                                                          AS user_key,
